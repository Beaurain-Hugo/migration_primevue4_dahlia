  ...
  return {
    ...
    async getUserById(id: number) {
     ...
     async getUser(url: string) {
      try {
        const response = await userApi.getUser(url);
        if (response) {
          return response; // Retourne les données utilisateur si l'appel est réussi
        } else {
          return 'Utilisateur non trouvé.'; // Gestion d'erreur : aucune donnée renvoyée
    ...
    async loginUser(email: string, password: string) {
      try {
        ...
        if (response) {
          localStorage.setItem('jwt', response.token); // Suppose que `token` est renvoyé par l'API
          try { 
            const res = await userApi.getMe();
            if(res.success){
              localStorage.setItem('id_user', res.data.id); // Suppose que `token` est renvoyé par l'API
            }
          } catch (error) {
    ...
    async createUserAccount(
      ...
      try {
        const response = await userApi.createUserAccount(firstName, lastName, email, password);
