...
import { BaseApiUrlAssociations, BaseApiUrlUsers, BaseApiUrlTresories, BaseApiUrlBudgets, BaseApiUrlActions, BaseApiUrlDocuments, ApiUrlDocuments } from '@/constants/ApiUrls';
import axios from 'axios';

export function useAssoApi() {
  return {
    async getAssociationById(id: number) {
      const res = await axiosInstance.get(`${BaseApiUrlAssociations}/${id}`);
      return res.data;
    },
    ...
    async createAssociation(data: any) {
      const res = await axiosInstance.post(`${BaseApiUrlAssociations}`, data, {headers: {'Content-Type': 'application/ld+json', 'Accept': 'application/ld+json'}
      });
      return res.data;
    },
